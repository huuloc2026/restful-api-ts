...
class AuthController {
    ...
    static refreshAccessToken = async (req, res, next) => {
        ...
        // console.log(token);
        if (!token) {
            return responseHandler.error(res, 403, "Refresh token required", "Fail refresh new access token");
        }
        try {
            ...
            if (typeof user === "object" && "userId" in user) {
                ...
                return responseHandler.success(res, 201, { accessToken: newAccessToken }, "Refresh:: Access Token successfully");
            }
            else {
                return responseHandler.error(res, 403, "Refresh Token invalid ! Please Log in", "Fail refresh new access token");
            }
        }
        catch (err) {
            return responseHandler.error(res, 403, err, "Fail refresh new access token");
        }
    };
    static logout = (req, res) => {
        // console.log(req.cookies.refreshToken);
        res.clearCookie("refreshToken", { path: "/auth/refresh" });
        responseHandler.success(res, 201, "Log out successfully and clear all", "Log out successfully and clear all");
    };
}
